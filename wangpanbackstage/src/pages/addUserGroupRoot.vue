...
<div class="main bgf7f8f9">
    <div class="bgfff h-con">
        <div class="h222 pt44 pl68 bt-eaebef fsize14 col333">
            ...
            <p class="fsize24 f-pf2 mt30 pl40 fweight600">新增用户组</p>
            <div class="flex mt28 pl40">
                <p class="col57 mr28 f-pf1">用户组名称</p>
                <input class="wid360 h40 fsize14 col333 bor-rad4 bor-c4c7cc pl20 pr20" type="text" placeholder="File name" v-model="username">
            </div>
        </div>
        ...
        <!-- <div class="pl108 pr68 pt28 pb20 col333 fsize14 bt-eaebef">
            ...
            <ul class="mt30">
                    <li class="mb20 pr10" v-for="(item,index) in list" :key="index">
                        <div class="flex cursor" @click="open1(item.children,item.open,item.id,item.isDir,index)">
                        ...
                        <ul class="pl20" v-if="item.children&&item.open"> 
                                <li class="mb10" v-for="(item2,index2) in item.children" :key="index2">
                                    <div class="flex cursor bl-cd" :class="item2.isDir==1?'pl10':'pl28'" @click="open2(item2.children,item2.open,item2.id,item2.isDir,index,index2)">
                                    ...
                                    <ul class="pl20" v-if="item2.children&&item2.open">
                                            <li class="mb10" v-for="(item3,index3) in item2.children" :key="index3">
                                                <div class="flex cursor bl-cd" :class="item3.isDir==1?'pl10':'pl28'" @click="open3(item3.children,item3.open,item3.id,item3.isDir,index,index2,index3)">
                                                ...
                                                <ul class="pl20" v-if="item3.children&&item3.open">
                                                    <li class="mb10" v-for="(item4,index4) in item3.children" :key="index4">
                                                        <div class="flex cursor bl-cd" :class="item4.isDir==1?'pl10':'pl28'" @click="open4(item4.children,item4.open,item4.id,item4.isDir,index,index2,index3,index4)">
                                                        ...
                                                        <ul class="pl20" v-if="item4.children&&item4.open">
                                                            <li class="mb10" v-for="(item5,index5) in item4.children" :key="index5">
                                                                <div class="flex cursor bl-cd" :class="item5.isDir==1?'pl10':'pl28'" @click="open5(item5.children,item5.open,item5.id,item5.isDir,index,index2,index3,index4,index5)">
...
export default {
    ...
    methods:{
        ...
        //二层数据
         open1(children,open,id,dir,ind){
            if(dir==1){
                if(open){       //关闭
                    let list = [...this.list];
                    list[ind].open = false;
                    ...
                    if(children&&children.length>0){
                        ...
                        this.$fetch({
                            ...
                            if(res.data.returnCode=='0000'){
                                let list = [...this.list];
                                list[ind].children = res.data.data.list;
                                list[ind].open = true;
                                this.list = list;
                            }else{
                                this.$message(res.data.msg);
        ...
        //三层数据
        open2(children,open,id,dir,ind,ind2){
            if(dir==1){
                if(open){       //关闭
                    let list = [...this.list];
                    ...
                    if(children&&children.length>0){
                        ...
                        this.$fetch({
                            ...
                            if(res.data.returnCode=='0000'){
                                let list = [...this.list];
                                list[ind].children[ind2].children = res.data.data.list;
                                list[ind].children[ind2].open = true;
                                this.list = list;
                            }else{
                                this.$message(res.data.msg);
        ...
        //四层数据
        open3(children,open,id,dir,ind,ind2,ind3){
            if(dir==1){
                if(open){       //关闭
                    let list = [...this.list];
                    ...
                    if(children&&children.length>0){
                        ...
                        this.$fetch({
                            ...
                            if(res.data.returnCode=='0000'){
                                let list = [...this.list];
                                list[ind].children[ind2].children[ind3].children = res.data.data.list;
                                list[ind].children[ind2].children[ind3].open = true
                                this.list = list;
                            }else{
                                this.$message(res.data.msg);
        ...
        //五层数据
        open4(children,open,id,dir,ind,ind2,ind3,ind4){
            if(dir==1){
                if(open){       //关闭
                    let list = [...this.list];
                    ...
                    if(children&&children.length>0){
                        ...
                        this.$fetch({
                            ...
                            if(res.data.returnCode=='0000'){
                                let list = [...this.list];
                                list[ind].children[ind2].children[ind3].children[ind4].children = res.data.data.list;
                                list[ind].children[ind2].children[ind3].children[ind4].open = true;
                                this.list = list;
                            }else{
                                this.$message(res.data.msg);
        ...
        //六层数据
        open5(children,open,id,dir,ind,ind2,ind3,ind4,ind5){
            if(dir==1){
                if(open){       //关闭
                    let list = [...this.list];
                    ...
                    if(children&&children.length>0){
                        ...
                        this.$fetch({
                            ...
                            if(res.data.returnCode=='0000'){
                                let list = [...this.list];
                                list[ind].children[ind2].children[ind3].children[ind4].children[ind5].children = res.data.data.list;
                                list[ind].children[ind2].children[ind3].children[ind4].children[ind5].open = true;
                                this.list = list;
                            }else{
                                this.$message(res.data.msg);
        ...
        Confirm(){
            ...
            // return;
            this.$fetch({
                url:'admin/admin_user/add_group',
                type:'post',
                data
            }).then(res=>{
                if(res.data.returnCode=='0000'){
                    this.$message({type:'success',message:'用户组添加成功'});
                    this.$router.back(-1);
                }else{
                    this.$message(res.data.msg);
                }
